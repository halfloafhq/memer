...
export function usePostMemeToCollection() {
  ...
  const postMemeToCollection = useCallback(
    async (memeId: string, collectionId: string, memeName: string, collectionName: string) => {
      try {
        setLoading(true);
        ...
        return toast({
          title: 'Save Failed',
          description: 'There was an error saving the meme to the collection. Please try again.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
